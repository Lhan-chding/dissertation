...
from src.optimizer_fork import capture_state, restore_state, state_hash, save_checkpoint, load_checkpoint
...
class TorchBoundaryTests(unittest.TestCase):
    ...
    def test_fixed_denominator_gradient_is_sequence_score_over_64(self):
        ...
        loss, audit = torch_ppo_loss(new, new.detach().clone(), torch.tensor([1., -1.]), mask)
        loss.backward()
        torch.testing.assert_close(new.grad, torch.tensor([[-1/128, -1/128, 0], [1/128, 0, 0]]))
    ...
    def test_adam_rng_forks_and_disk_resume_match_uninterrupted(self):
        ...
        self.assertEqual(state_hash(capture_state(model, optimizer, origin['metadata'])), origin_hash)
        update()
        self.assertEqual(state_hash(capture_state(model, optimizer, {'step': 2})), state_hash(expected))
