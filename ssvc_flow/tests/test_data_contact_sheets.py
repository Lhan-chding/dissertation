...
def test_contact_sheets_preserve_all_cells_and_review_status(tmp_path):
    ...
    for sheet in manifest['sheets']:
        ...
        cells = Counter((row['constraint_family'], row['chart_type'], row['operation']) for row in sheet['scenes'])
        assert len(cells) == 18 and set(cells.values()) == {1}
        assert all('truth_world' not in row and 'observed_world' not in row for row in sheet['scenes'])
    assert len({row['base_scene_id'] for sheet in manifest['sheets'] for row in sheet['scenes']}) == 36
