...
def test_categories_partition_and_nested_events():
    ...
    assert (
        annotate("[1,2,3,5]", truth, "sum4", {"family": "trend"})["constraint_satisfaction"]
        is False
    )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            [2, 9, 3, 7],
            [2, 8, 3, 7],
            {"family": "duplicate_encoding", "known_index": 1, "known_value": 9},
        ),
        (
            [2, 9, 3, 7],
            [2, 9, 6, 7],
            {"family": "cross_series", "edges": [[0, 1, 11], [0, 2, 5], [0, 3, 9]]},
        ),
        (
            [2, 9, 3, 7],
            [2, 9, 6, 7],
            {"family": "cross_series", "edges": [[0, 1, 11], [1, 2, 12], [2, 3, 10]]},
        ),
    ],
)
...
def test_no_cue_is_396_candidates_and_diagnostics_not_coerced():
    ...
    assert (
        solve([1, 2, 3, 4], {"family": "duplicate_encoding", "known_index": 0, "known_value": 101})
        == []
    )
    multiple = solve(
        [1, 2, 3, 4], {"family": "duplicate_encoding", "known_index": 0, "known_value": 1}
    )
