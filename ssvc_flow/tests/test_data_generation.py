...
def test_default_split_contract():
    assert DEFAULT_SIZES == {'calibration': 144, 'train': 576, 'control': 144, 'dev': 144, 'confirm': 288, 'ood': 288, 'natural_pool': 2048}


def test_balanced_unique_deterministic_and_unleaked_generation(tmp_path):
    sizes = {'calibration': 18, 'train': 36, 'control': 18, 'dev': 18, 'confirm': 18, 'ood': 18, 'natural_pool': 18}
    out = tmp_path / 'first'
    ...
    for split, count in sizes.items():
        ...
        for scene in scenes:
            ...
            assert sum(a != b for a, b in zip(scene['truth_world'], scene['observed_world'])) == 1
            assert scene['solution_count'] == 1
            assert len(scene['base_scene_id']) > 10
            assert all(str(value) not in scene['image_path'].split('/')[-1].split('_') for value in scene['truth_world'])
        if split in {'calibration', 'train', 'control', 'dev', 'confirm'}:
            cells = Counter((r['constraint_family'], r['chart_type'], r['operation']) for r in scenes)
            assert len(cells) == 18 and len(set(cells.values())) == 1
            assert all(max(r['truth_world']) <= 49 for r in scenes)
        if split == 'train':
            assert Counter(r['interface'] for r in scenes) == {'SYMBOLIC_FRESH': 18, 'IMAGE_CUE_FRESH': 18}
            assert len(set(Counter((r['constraint_family'], r['chart_type'], r['operation'], r['interface']) for r in scenes).values())) == 1
    ood = read_scenes(out, 'ood')
    assert Counter(r['ood_subtype'] for r in ood) == {'graph_structure': 6, 'numeric_shift': 6, 'render_expression': 6}
    assert all(r['constraint_family'] == 'cross_series' and r['graph_topology'] in {'path', 'cycle'} for r in ood if r['ood_subtype'] == 'graph_structure')
    ...
    assert manifest['files']['train.jsonl']['sha256'] == hashlib.sha256((out / 'train.jsonl').read_bytes()).hexdigest()


@pytest.mark.parametrize('sizes', [{'train': 18}, {'calibration': 19}, {'natural_pool': 2049}, {'ood': 17}, {'bad': 1}])
...
def test_fixed_prompt_templates_ignore_truth_fields():
    scene = {'observed_world': [12, 2, 3, 4], 'cue': {'family': 'duplicate_encoding', 'known_index': 0, 'known_value': 1}, 'operation': 'sum4', 'image_path': 'images/opaque.png', 'truth_world': [1, 2, 3, 4], 'executor_answer': 10}
    ...
    assert cue_text({'family': 'cross_series', 'edges': [[2, 3, 7], [0, 1, 3]]}) == 'a + b = 3\nc + d = 7'
...
def test_rendered_dataset_records_hashes_and_diagnostics(tmp_path):
    ...
    assert first['image_hash'] == hashlib.sha256((tmp_path / first['image_path']).read_bytes()).hexdigest()
