...
目前仅完成 CPU 和单卡 smoke，未提交双卡实验、S1 或 S2。双卡作为具备独立执行单元时的资源目标，每个进程只可见一张 GPU；不为等待后续门禁而闲占第二张卡。下一门禁仍是按交接顺序完成六个 S1 bank、检查各自原始结果并生成实测技术链；随后才审核 S2 技术链、预算与训练授权。smoke PASS 不替代这些门禁。
