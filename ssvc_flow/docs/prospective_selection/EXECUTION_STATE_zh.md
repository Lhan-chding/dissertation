...
- P0已重读36,864条旧P输出；16个旧E端点评估已开始：O2_R0作业169620首次核验RUNNING，其余15个尚未提交。实际端点列表由 history.HISTORICAL_ENDPOINTS 提供（O1十配方，O2六配方）。
...
下一步：核验169616–169620启动与原始进度；满5个项目单PRO6000作业时不再提交。完成/释放槽位后按持久队列继续，并先交付首4条lineage及E复核/实测工期再扩展。任何实际故障先核对权威完成段、日志和Slurm终态，保留失败attempt；未知提交不重提，数值非有限不能盲目重启。
