...
修正后的分析代码独立部署为 `code_905f6a8`，真实原件服务器 CPU 验证已完成。
...
- CPU 作业 **158483**：`COMPLETED`，退出 `0:0`，37 秒，实际 2 CPU / 6 GiB / 无 GPU。
- 读取两个 B 原点，24 个原点×题目单元均完成身份校验，24 个条件支持界均有效；
  这些界仍以已记录数值评分为条件，不包含 BF16 系统误差保证。
- 与已完成桥接分析比较 288 个 ORIGIN/MIX/COUNT 点估计：最大差 `5.21e-17`；
  方差最大差 `8.68e-19`；192 个单事件 COUNT 区间端点差为 0。
- 没有新增测量模型调用，没有修改原件或绕过源码身份检查。
- 2026-09-17 10:36 UTC 附近的核验：158482、157057 均为 RUNNING；
  各一张 PRO6000，账户 `rose`，指定教师 QoS；seed41001 已初始化（保存步0），
  seed41002 保存到步11。后续状态由15分钟只读监控跟踪。

服务器结果：`analysis/measurement_905f6a8_20260917/`。
本地结果与核验收据：`reports/modeling_v4/measurement_905f6a8_20260917/VERIFICATION.json`。
本次完成现有原件的独立测量报告接入；旧 campaign 的后续拟合源码版本适配仍未执行，
详见 `BRIDGE_ERROR_ANALYSIS.md`。科学状态仍为 `NOT_CERTIFIED`。
