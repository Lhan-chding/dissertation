...
PHASE6_LOCKED_PATHS = (
    ...
    "scripts/v4/12_prepare_phase6_rl_data.py",
    "scripts/v4/13_train_phase6_grpo.py",
    "scripts/v4/14_evaluate_phase6_rl.py",
...
def _require_sha256_mapping(
    ...
    *,
    ...
    if not isinstance(value, dict) or tuple(sorted(value)) != tuple(sorted(expected_keys)):
        raise ValueError(f"{label} keys differ from the frozen contract")
    return {key: _require_sha256(value[key], f"{label} {key}") for key in expected_keys}
...
def load_phase6_config(path: Path) -> Phase6PlanConfig:
    ...
    if (
        ...
        or payload.get("status") != "PHASE_6_GRPO_AUTHORIZED"
    ):
    ...
    if payload.get("authorization") != {
        ...
        "server_training_authorized": True,
        "rl_authorized": True,
    ...
    if gates != {
        ...
        "require_reward_trace_checkpointing": True,
    ...
    if training != {
        ...
        raise ValueError("Phase 6 training contract drifted")
...
def load_phase5_policy_support_summary(path: Path, *, expected_sha256: str) -> dict[str, object]:
    ...
    if (
        model_snapshot_sha256 is not None
        and _require_sha256(model_snapshot_sha256, "Phase 5 model snapshot")
...
def build_phase6_execution_manifest(
    *,
    ...
    manifest = {
        ...
        "package_lock_sha256": _require_sha256(package_lock_sha256, "Phase 6 package-lock hash"),
...
def load_phase6_execution_manifest(
    path: Path,
    *,
    expected_sha256: str,
    ...
    """Load and verify the manifest that authorizes downstream Phase 6 execution."""

    if path.is_symlink() or not path.is_file():
        raise ValueError("Phase 6 execution manifest must be a regular JSON file")
    _require_sha256(expected_sha256, "Phase 6 execution manifest SHA-256")
    _require_sha256(expected_config_sha256, "Phase 6 config SHA-256")
    _require_sha256(expected_package_lock_sha256, "Phase 6 package-lock SHA-256")
    if hashlib.sha256(path.read_bytes()).hexdigest() != expected_sha256:
        raise RuntimeError("Phase 6 execution manifest SHA-256 mismatch")
    payload = json.loads(path.read_text(encoding="utf-8"))
    arms = payload.get("arms") if isinstance(payload, dict) else None
    source_hashes = payload.get("source_sha256") if isinstance(payload, dict) else None
    phase4_hashes = payload.get("phase4_adapter_sha256") if isinstance(payload, dict) else None
    if (
        not isinstance(payload, dict)
        or payload.get("schema_version") != 1
        or payload.get("artifact_type") != "v4_phase6_execution_manifest"
        or payload.get("status") != "PHASE_6_RL_EXECUTION_MANIFEST_PREPARED"
        or payload.get("config_sha256") != expected_config_sha256
        or payload.get("package_lock_sha256") != expected_package_lock_sha256
        or payload.get("scene_is_statistical_unit") is not True
        or payload.get("subjective_success_threshold_applied") is not False
        or payload.get("confirmatory_data_used") is not False
        or payload.get("training_invoked") is not False
        ...
        or tuple(item.get("name") for item in arms if isinstance(item, dict))
        != (
            "Base",
            "Base_AnswerOnly_RL",
            "Recovery_LoRA",
        ...
        raise ValueError("Phase 6 execution manifest differs from the frozen contract")
    _require_sha256(payload.get("phase5_policy_support_summary_sha256"), "Phase 6 Phase 5 summary")
    _require_sha256(payload.get("model_snapshot_sha256"), "Phase 6 model snapshot")
    _require_sha256_mapping(
        source_hashes,
        label="Phase 6 source hashes",
        expected_keys=("Base", "C0", "C1", "T", "support_dev"),
    )
    _require_sha256_mapping(
        phase4_hashes,
        label="Phase 6 Phase 4 adapter hashes",
        expected_keys=("C0", "C1", "T"),
    )
    expected_arms = (
        ("Base", "Base", "none", False),
        ("Base_AnswerOnly_RL", "Base", "answer_only", True),
        ("Recovery_LoRA", "T", "none", False),
        ("Recovery_LoRA_RecoveryOutcome_RL", "T", "recovery_outcome", True),
        ("Recovery_LoRA_AnswerOnly_RL", "T", "answer_only", True),
    )
    for arm, expected in zip(arms, expected_arms, strict=True):
        if not isinstance(arm, dict):
            raise ValueError("Phase 6 execution manifest arm entry is malformed")
        name, checkpoint, reward_mode, execute_rl = expected
        if (
            arm.get("name") != name
            or arm.get("initialization_checkpoint") != checkpoint
            or arm.get("reward_mode") != reward_mode
            or arm.get("execute_rl") is not execute_rl
        ):
            raise ValueError("Phase 6 execution manifest arm contract drifted")
        _require_sha256(
            arm.get("initialization_sha256"),
            f"Phase 6 execution manifest initialization hash {name}",
        )
    return payload
