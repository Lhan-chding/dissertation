...
from compensability_v5.study_c2.visibility import visibility_bin
...
def test_policy_weighted_fiber_purity_and_update_decomposition() -> None:
    ...
    assert update["delta_exact_mass"] == pytest.approx(0.05)
    assert update["delta_answer_success_mass"] == pytest.approx(0.25)
...
def test_reward_visibility_and_counterfactual_operation_matrix() -> None:
    ...
    assert [visibility_bin(value) for value in (0, -1, 3)] == [
        "reward_null",
        "low_visibility",
        "high_visibility",
    ]
    with pytest.raises(ValueError, match="at least one operation"):
        counterfactual_reward_matrix(actions=(truth,), truth=truth, operations=())
...
def test_correction_vector_geometry_separates_correct_edit_copy_and_overedit() -> None:
    ...
    overedit = correction_vector_metrics(
        truth=(4, 5, 6, 7), observation=(3, 5, 6, 7), candidate=(4, 4, 6, 7), operation=operation
    )
