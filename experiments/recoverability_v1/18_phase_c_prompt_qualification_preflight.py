#!/usr/bin/env python3
"""Metadata-only preflight for the 36-call Phase-C prompt qualification."""
...
def _lock_rows(path: Path) -> tuple[tuple[str, str], ...]:
    ...
    if len(paths) != len(digests) or not paths or len(paths) != len(set(paths)):
        raise SystemExit("BLOCKED: Phase C prompt qualification package lock is malformed")
...
def main() -> int:
    ...
    config_path = root / "configs/recoverability/phase_c_prompt_qualification_v1.yaml"
    if args.runtime.resolve() != runtime or args.runtime.is_symlink():
        raise ValueError("prompt qualification runtime path is not canonical")
    if args.server_package_lock.resolve() != lock or args.server_package_lock.is_symlink():
        raise ValueError("prompt qualification package lock path is not canonical")
    if args.output.exists() or args.output.is_symlink():
        raise FileExistsError(
            f"refusing to overwrite prompt qualification preflight: {args.output}"
        )
    if args.output.parent.is_symlink() or not args.output.parent.is_dir():
        raise ValueError("prompt qualification preflight parent must be a regular directory")
    ...
    config = load_phase_c_prompt_qualification_config(config_path)
    package = verify_phase_c_prompt_qualification_package_lock(lock, repository_root=root)
    ...
    if not report.ready or report.large_gpu_started or report.model_loaded:
        raise RuntimeError("prompt qualification metadata preflight did not remain inert")
    payload = {
        **asdict(report),
        "artifact_type": "recoverability_phase_c_prompt_qualification_metadata_preflight",
        "schema_version": 1,
