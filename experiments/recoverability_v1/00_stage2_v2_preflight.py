#!/usr/bin/env python3
"""Run metadata-only checks for the Stage-2 v2 development probe."""
...
def main() -> int:
    ...
    canonical_runtime = root / "configs/recoverability/server_runtime_v1.yaml"
    if args.runtime.resolve() != canonical_runtime or args.runtime.is_symlink():
        raise ValueError("Stage-2 v2 runtime path is not canonical")
    if args.output.exists() or args.output.is_symlink():
        raise FileExistsError(f"refusing to overwrite Stage-2 v2 preflight: {args.output}")
    if args.output.parent.is_symlink() or not args.output.parent.is_dir():
        raise ValueError("Stage-2 v2 preflight output parent must be a regular directory")
    package_lock = verify_stage2_v2_server_package_lock(
