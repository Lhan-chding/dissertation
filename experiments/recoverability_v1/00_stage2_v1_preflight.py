#!/usr/bin/env python3
"""Run metadata-only checks for the Stage-2 v1 development probe."""
...
def main() -> int:
    ...
    if args.output.exists() or args.output.is_symlink():
        raise FileExistsError(f"refusing to overwrite Stage-2 v1 preflight: {args.output}")
    if args.output.parent.is_symlink() or not args.output.parent.is_dir():
        raise ValueError("Stage-2 v1 preflight output parent must be a regular directory")
    package_lock = verify_stage2_v1_server_package_lock(
